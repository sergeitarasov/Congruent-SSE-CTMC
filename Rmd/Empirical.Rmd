...
#### Gridy search for the best SEM (NOT INCLUDED)
...
# [1] -984.2505
# [1] "matrix:  1 2 2"
# [1] "Recoding: 0010 -> 0.9250.9250.9250.9250.9250.9250.9250.9250.07500.07500.07500.0750"
# [1] "Recoding: 0001 -> 0.9250.9250.9250.9250.9250.9250.9250.92500.07500.07500.07500.075"

result[[1]]$lnLik
lapply(result, function(x) x$lnLik) %>% unlist

ar <- pars_to_arrays(best$par.full, 8)
Qpl <- names2array(ar, c('a0', 'a1', 'b0', 'b1', 'c0', 'c1', 'd0', 'd1'))  %>% reoder_lambdas(., c(1,3,5,7, 2,4,6,8))
Qpl <-Qpl$Q
g <- create_graph_from_matrix(Qpl)
unidirectional_edges <- is_unidirectional(g)
plot(g, vertex.label = V(g)$name, vertex.size = 30, edge.color = ifelse(unidirectional_edges, "red", "black"))
...
#### Independent SEM (NOT INCLUDED)

```{r eval=FALSE, include=FALSE}
# trait
Q4 <- initQ(c('a','b', 'c', 'd'), c(1, 1))
...
# Plot the graph
g <- create_graph_from_matrix(Q8.t)
unidirectional_edges <- is_unidirectional(g)
plot(g, vertex.label = V(g)$name, vertex.size = 30, edge.color = ifelse(unidirectional_edges, "red", "black"))
