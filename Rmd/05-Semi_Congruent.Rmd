...
# Define blocks to be used for Q construction
blocks <- list(
...
# # Define index
# index <- list(
#   matrix(c(1:2, 3:4,  1:2, 5:6), 2, 4, byrow = TRUE),
#   matrix(c(3:4, 1:2,  3:4, 7:8), 2, 4, byrow = TRUE),
#   matrix(c(5:6, 1:2,  5:6, 7:8), 2, 4, byrow = TRUE),
#   matrix(c(7:8, 3:4,  7:8, 5:6), 2, 4, byrow = TRUE)
# )
# Define index
index <- list(
  matrix(c(1:2, 3:4,  1:2, 5:6,  1:2, 7:8), 3, 4, byrow = TRUE),
...
# # let's imagine that the number of pars is 4
# noncong.aic4par <- tibble(
# t100=get_item(readRDS( file='R/data/non-congruent/best_results-EHE8_phy_CID4-100tips-100tr.RDS'), 'lnLik') %>% get_aic(., 4),
# t500=get_item(readRDS( file='R/data/non-congruent/best_results-EHE8_phy_CID4-500tips-100tr.RDS'), 'lnLik') %>% get_aic(., 4),
# t1000=get_item(readRDS(file='R/data/non-congruent/best_results-EHE8_phy_CID4-1000tips-100tr.RDS'), 'lnLik') %>% get_aic(., 4)
# )
...
combined_ecdf_data <- bind_rows(
  ecdf_data_nc %>% mutate(type = 'noncong'),
  ecdf_data %>% mutate(type = 'semicong')
)

# ecdf_data_nc <- ecdf_data_nc %>% mutate(type = 'noncong')
# ecdf_data <- ecdf_data %>% mutate(type = 'semicong')
```
...
d_noncong <- combined_ecdf_data %>% filter(type == "noncong")
d_semicong <- combined_ecdf_data %>% filter(type == "semicong")
...
cdf_plot <- ggplot() +
  stat_ecdf(data = d_noncong, aes(x = Value, linetype = Group, color = type), geom = "step", pad = F, size = .4) +
