...
derivs.classeNew <- function (t, y, pars)
{
...
initial.conditions.classeNew <- function(init, pars, t, idx) {
  #print(init)
...
rootfunc.classeNew <- function (res, pars, condition.surv, root, root.p, intermediates){
  vals <- res$vals
  lq <- res$lq
  #d.root <- vals[3:4]
...
make.info.classeNew <- function (phy, newArgs)
{
  list(name = "new_classe", name.pretty = "New_ClaSSE", np = as.integer(length(argnames_HiClaSSE(newArgs$Nstates)$pars)),
       argnames = argnames_HiClaSSE(newArgs$Nstates)$pars,
       ny = newArgs$ny, k = newArgs$Nstates, idx.e = newArgs$idx.e, idx.d = newArgs$idx.d, derivs = derivs.classeNew,
...
make.cache.classeNew <- function (tree, states, sampling.f = NULL, strict = TRUE, newArgs)
{
  cache <- diversitree:::make.cache.musse(tree, states + 1L, 2L, sampling.f, strict)
  cache$info <- make.info.classeNew(tree, newArgs)
  cache
}


make.classeNew <- function (tree, states, sampling.f = NULL, strict = TRUE, control = list(), newArgs=list()) {
  cache <- make.cache.classeNew(tree, states, sampling.f, strict, newArgs)
  ...
  all_branches <- diversitree:::make.all_branches.dtlik(cache, control, initial.conditions.classeNew)
  #rootfunc <- diversitree:::rootfunc.geosse
  #rootfunc <- diversitree:::rootfunc.musse
  rootfunc <- rootfunc.classeNew
